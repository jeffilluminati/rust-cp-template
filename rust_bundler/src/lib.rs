...
impl<'a> Expander<'a> {
    ...
    fn set_pub_mod_allow_list(&mut self, file: &syn::File) {
        ...
        let mut collector = UsedRootCollector::new(self.crate_name);
        let mut file = file.clone();
        collector.visit_file_mut(&mut file);
        self.allow_list_mod_in_lib.extend(collector.finish());
        for it in &file.items {
            if let syn::Item::Use(item_use) = it {
                self.allow_list_mod_in_lib.extend(extract_root_mods_from_use_tree(
                    &item_use.tree,
                    self.crate_name,
                    false,
                ));
...
fn ensure_selected_extern_crate(file: &mut syn::File, crate_name: &str) {
    ...
    if !has_path_reference && !has_use_reference {
        return;
    }
